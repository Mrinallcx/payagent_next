...
const MOCK_PAYMENT_LINK = {
  ...
  token: "ETH",
  networks: ["ETH", "BASE"],
  // Multiple networks from CreateLinkModal
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {paymentData.networks.map(network => <button key={network} onClick={() => setSelectedNetwork(network)} className={`w-full p-5 rounded-xl border-2 transition-all duration-200 ${selectedNetwork === network ? "border-[#0B6FFE] bg-[#F8FBFF] shadow-md" : "border-[#E8F0FF] hover:border-[#80A9FF] hover:bg-[#FAFBFC]"}`}>
              ...
              <Button className="w-full h-14 text-base font-semibold rounded-xl shadow-md hover:shadow-lg transition-all" onClick={handleContinueToPayment} disabled={!selectedNetwork}>
        ...
        {step === "payment" && <Card className="p-6 border border-[#E8F0FF] shadow-sm">
            <div className="space-y-6">
              ...
              <div>
                ...
                <Input placeholder="Enter transaction hash (0x...)" value={transactionHash} onChange={e => setTransactionHash(e.target.value)} className="font-mono text-sm" />
              </div>

              <Button className="w-full h-12 text-base font-medium" onClick={handlePaymentDone} disabled={!transactionHash}>
                ✓ I've sent payment
              </Button>
            </div>
          </Card>}
